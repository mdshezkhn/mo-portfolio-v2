...
## 0.5 User Answers Received (2026-09-13)

1. **Legal/passport name** — Legal name is **Khan Mohammed Shehzad Anwar** (Mohammed Shehzad is the given name, Anwar is the father's name, Khan is the surname). Update site and `cv.html` to the legal name; keep all certificate variants since they refer to the same person.
2. **Zhejiang University affiliation** — Keep the "Zhejiang University — Helen China TEFL Network" affiliation; it was the first job in China.
3. **Harris University M.A.** — The degree has no accreditation but supported eligibility for a Chinese work visa. Retain it with the existing note: "External recognition status unverified; retained for completeness as it supported eligibility for a China work visa."
4. **Safeguarding training** — No formal training beyond the UNICEF certification ("What We Stand For: Essentials of Children's Rights"). Add a brief note under Credentials.
5. **QTS pathway plan** — Holds a B.Ed from Kashmir University and a PGCE from the University of Cumbria (non-QTS). Not interested in pursuing QTS. Add one honest sentence: "I hold a Postgraduate Certificate in Education (PGCE, non-QTS) from the University of Cumbria and a B.Ed. from the University of Kashmir; I am not currently pursuing Qualified Teacher Status."
6. **Email addresses** — Keep both `mdshezkhn@hotmail.com` and `mdshezkhn@gmail.com`. Update both the site and `cv.html`.
7. **Portrait** — No new portrait available. Use the existing thumbs-up portrait with an `onerror` fallback to a styled monogram; remove any white-wash flattening.
8. **Teaching video** — OneDrive link provided: `https://1drv.ms/v/c/b64d3b12668bb397/IQBB5r2X1-kRRaQaHI-YaF67AS7kjfPeQHIAsxb-AohDmGo?e=k0R6Ai` — Grade 5 reading lesson. Add a "See Me Teach" CTA with year group "Grade 5", subject "Reading", and "what-to-watch-for" notes. No autoplay.
9. **Gallery / Classroom photos** — Three OneDrive photo links shared. Create slots in `assets/images/gallery/` with naming convention `gallery-01.webp`, `gallery-02.webp`, `gallery-03.webp`. User will download and place the actual files. Each slot should have `loading="lazy"`, descriptive alt text, and be under 150 KB.
10. **Research evidence** — Two OneDrive research links shared. Create placeholder sections for three flagship case studies; the user will provide or confirm the specific evidence. The PGCE practitioner research is documented; "EAL + STEM" and "Developing Teachers" will have placeholder CTA links until artifacts are supplied.
11. **Impact metrics** — The CV states "more than 200 educators" trained at GEDU — confirmed correct for use on the site. GEDU was a UK-based organisation based in Noida; work involved offline training for local educators and online training for international (UK, Malta, Dubai) educators. No other verified statistics beyond the 11+ years, Years 1-6, and 200+ educators.
12. **Availability** — "Available August 2027" is accurate. Keep as-is.
13. **Languages** — Six languages: English (first language), Urdu, Hindi, Marathi, Arabic (can read), Mandarin (basic conversation). Add under Credentials or a visual strip near the hero.
14. **Footer nav + social links** — Applicant wants the portfolio to be "eye-catching and act as a recruiter magnet" and to "highlight technical prose on top of language skills." Keep LinkedIn + email in the footer, add a GitHub link (repo is public), and keep WeChat/WhatsApp behind a secondary "China-based recruitment" option.
15. **Design tokens / spacing** — Proceed with the existing light/navy theme as-is. Priority is a smooth, feather-weight load on phones and handheld devices. No new `--space-*` token extraction needed; keep CSS lean.
16. **Other content or constraints** — Applicant wants the portfolio to be "eye-catching and act as a recruiter magnet" and also wants to "highlight my technical prose on top of my language skills based on which I was able to come up with the portfolio and about my skill to use AI effectively." The site should reflect a clean, professional, evidence-led design with personality — no glassmorphism, no glowing effects, no animated counters. Animation serves hierarchy only; respect `prefers-reduced-motion`.

---

## 0.6 OneDrive Download Status

The three gallery photo links, teaching video link, and two research links are OneDrive share links that require authentication to download programmatically. The repository will use the documented naming convention and placeholder sections until the user downloads and places the actual files in the appropriate asset folders.
